...
contract UniswapV3SwapTest is Test {
    ...
    function setUp() public {
        deal(DAI, address(this), 1000 * 1e18); // dealing to this test contract 1000 DAI, Where 1 DAI = 1e18
        dai.approve(address(router), type(uint256).max); // approving the swap router 02 contract to manage all the DAI in this contract
    }
    ...
    function test_exactInputSingle() public {
        ...
        // Calling router.exactInputSingle
        ...
        console2.log("WETH amount out %e", amountOut); // amountOut is in __ decimals, di,
    ...
    function test_exactInput() public {
        // Calling router.exactInput

        // DAI, 3000 fees, WETH = specifies the DAI / WETH pool with the 0.3%(3000) fee tier
        // WETH 3000 fees, WBTC = sepcifies the WETH / WBTC pool with the 0.3% fee tier
    ...
    //  3
    // Swap maximum of 1000 DAI to obtain exactly 0.1 WETH from DAI/WETH pool with 0.35 fee
    // Send WETH from Uniswap V3 to this contract
    function test_exactOutputSingle() public {
        ...
        assertEq(wethAfter - wethBalance, 0.1 * 1e18);
    }
    ...
    // - Swap DAI to WETH on pool with 0.3% fee
    // - Swap WETH to WBTC on pool with 0.35 fee
    // - Send WBTC from Uniswap V3 to this contract
    // NOTE: WBTC has 8 decimals
