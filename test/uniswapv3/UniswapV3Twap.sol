...
error UniswapV3Tswap__InvalidToken();
...
contract UniswapV3Twap {
    ...
    returns (
        ...
        // Require tokenIn is token0 or token1
        if (tokenIn != token0 && tokenIn != token1){
            revert UniswapV3Tswap__InvalidToken();
        }

        // Assign tokenOut
        address tokenOut = tokenIn == token0 ? token1: token0;

        // Fill out timeDeltas with dt and 0
